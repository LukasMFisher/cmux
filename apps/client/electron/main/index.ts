...
function registerSshIpcHandlers(): void {
  // List available sandboxes
  ipcMain.handle("cmux:ssh:list-sandboxes", async () => {
    const { spawn } = await import("node:child_process");

    return new Promise<{ ok: boolean; sandboxes?: SandboxInfo[]; error?: string }>((resolve) => {
      const dmux = spawn("dmux", ["ls"], {
      ...
      dmux.stdout.on("data", (data: Buffer) => {
        stdout += data.toString();
      });

      dmux.stderr.on("data", (data: Buffer) => {
        stderr += data.toString();
      });

      dmux.on("error", (err) => {
        mainWarn("Failed to spawn dmux ls", err);
        resolve({ ok: false, error: `Failed to run dmux: ${err.message}` });
      });

      dmux.on("close", (code) => {
        if (code !== 0) {
          mainWarn("dmux ls failed", { code, stderr });
          resolve({ ok: false, error: stderr || `dmux ls exited with code ${code}` });
          return;
        }

        try {
          const sandboxes = JSON.parse(stdout) as SandboxInfo[];
          resolve({ ok: true, sandboxes });
        } catch (parseError) {
          mainWarn("Failed to parse dmux ls output", parseError);
          resolve({ ok: false, error: "Failed to parse sandbox list" });
        }
      });
    });
  });

  // Open VS Code with Remote-SSH to a sandbox
  ipcMain.handle("cmux:ssh:open-vscode", async (_event, sandboxId: string) => {
    const { spawn } = await import("node:child_process");

    mainLog("Opening VS Code SSH to sandbox", { sandboxId });

    // First, ensure SSH config is set up by running dmux ssh-config
    // Then open VS Code with Remote-SSH
    const sshHost = `sandbox-${sandboxId}`;
    const remotePath = "/workspace";

    return new Promise<{ ok: boolean; error?: string }>((resolve) => {
      // Use VS Code CLI to open remote SSH connection
      // The format is: code --remote ssh-remote+<host> <path>
      ...
      vscode.stderr.on("data", (data: Buffer) => {
        mainWarn("VS Code stderr:", data.toString());
      });

      vscode.on("error", (err) => {
        mainWarn("Failed to spawn VS Code", err);
        resolve({ ok: false, error: `Failed to open VS Code: ${err.message}` });
      });

      // Don't wait for VS Code to close - it will stay open
      vscode.unref();
  ...
  // Setup SSH config for all sandboxes
  ipcMain.handle("cmux:ssh:setup-config", async () => {
    const { spawn } = await import("node:child_process");
    const { homedir } = await import("node:os");
    const { appendFile, readFile, mkdir } = await import("node:fs/promises");

    mainLog("Setting up SSH config for sandboxes");

    return new Promise<{ ok: boolean; error?: string }>((resolve) => {
      // Get the SSH config snippet from dmux
      const dmux = spawn("dmux", ["ssh-config"], {
        stdio: ["ignore", "pipe", "pipe"],
      });
      ...
      dmux.on("error", (err) => {
        mainWarn("Failed to spawn dmux ssh-config", err);
        resolve({ ok: false, error: `Failed to run dmux: ${err.message}` });
      });

      dmux.on("close", async (code) => {
        if (code !== 0) {
          mainWarn("dmux ssh-config failed", { code, stderr });
          resolve({ ok: false, error: stderr || `dmux ssh-config exited with code ${code}` });
          return;
        }
        ...
        try {
          ...
          // Check if config already has sandbox entries
          let existingConfig = "";
          try {
          ...
          // Check if we already have the sandbox config
          if (existingConfig.includes("Host sandbox-*")) {
            mainLog("SSH config already contains sandbox entries");
            resolve({ ok: true });
            return;
          }
          ...
          const configToAdd = "\n" + stdout;
          await appendFile(configPath, configToAdd, { mode: 0o600 });

          mainLog("SSH config updated successfully");
          resolve({ ok: true });
        } catch (fsError) {
          mainWarn("Failed to update SSH config", fsError);
          resolve({ ok: false, error: `Failed to update SSH config: ${fsError}` });
        }
      });
    });
  });
